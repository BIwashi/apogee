...
- PR #39 — `apogee menubar install / uninstall / status` register the menu bar companion as a **second** launchd unit (`dev.biwashi.apogee.menubar`) so it starts on every login independent from the collector daemon unit. `apogee onboard` gains a Menubar group (macOS only) that defaults to install on a fresh machine and re-install when the plist already exists; non-darwin hides the group. Uses a new `daemon.NewManagerWithLabel` + `daemon.MenubarConfig` seam that reuses every existing Install/Uninstall/Start/Stop/Restart/Status code path with a menubar-shaped plist (`LSUIElement`, `LimitLoadToSessionType=Aqua`, `KeepAlive=false`, menubar log basenames). See [`docs/menubar.md`](docs/menubar.md#install-as-a-login-item).
