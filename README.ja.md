...
# 2. 1 コマンドの対話式セットアップウィザードを実行する。
apogee onboard
```

`apogee onboard` は `~/.claude/settings.json` への hook 登録、
launchd / systemd user サービスとしての apogee インストール、
サマライザーの言語と任意のシステムプロンプトを DuckDB に書き込み、
任意で OTLP エンドポイントを設定、デーモンを起動、そしてダッシュボードを
ブラウザで開くところまでを 1 回のシェル実行でまとめて行います。各プロンプトの
デフォルトはディスク上の現在の状態から読み込むので、再実行しても安全です。
CI や Docker プロビジョニングでは `--yes` を、確認だけなら `--dry-run` を
指定してください。詳細は [`docs/onboard_ja.md`](docs/onboard_ja.md) を参照してください。

明示的にステップを分けたい場合は次の通りです。

```sh
# コレクターを起動し、このマシンの全プロジェクト向けに hook を一度だけ入れる。
apogee serve &
apogee init
