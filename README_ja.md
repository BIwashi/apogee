...
- **さっきのセッション全体では、何が起きた？** 四層構造の LLM サマライザが、ターン単位の recap（Haiku）、セッション単位のナラティブ rollup（Sonnet）、tier-3 の **フェーズナラティブ** — ターンを意味的なチャンク (implement / review / debug / plan / test / commit / delegate / explore) にグルーピングし、フェーズごとに headline、1〜3 文の narrative、key_steps を生成 —、そして tier-4 の **live-status** ブラブ（Haiku、セッション単位）— 実行中セッションについて "currently &lt;verb&gt;-ing &lt;noun&gt;" の 1 行サマリーを最新に保つ — をすべてローカルの `claude` CLI で生成します。Anthropic API キーは不要です。デフォルトモデルはハードコードされなくなりました — apogee は静的カタログから「現時点で利用可能な最安値」のモデルを tier 毎に選択し、Settings ページや `apogee onboard` ウィザードから上書きできます。
...
| 画面 / サーフェス | 内容 |
|---|---|
| Live ページ | フォーカスカード駆動のランディング画面。実行中ターンをヒーロー表示し、フレームグラフ・recap 見出し・現在の phase と tool・ターン詳細へ飛ぶ CTA を並べます。縦の triage レールには実行中ターンを持つセッションが attention 順に並びます。Focus しているセッションの Mission git graph がグリッドの下にインライン展開されるので、セッション詳細へ遷移しなくても「プランのどこにいるか」が一目で分かります。 |
| Sessions カタログ | 収集済みセッションの検索・フィルタ可能な一覧（Datadog の Service Catalog に相当）。tier-4 の live-status ワーカーが実行中セッションについて "currently doing X" の 1 行ブラブを定期更新するので、各行を開かずに今何が進行中かが分かります。 |
| Agents | エージェントごとのビュー。main / subagent の分割、呼び出し回数、滑走平均時間、親 → 子のツリー表示。各行は LLM 生成の title / role（Haiku）で始まるため、同じセッション内の 5 並列 main エージェントが同じ見た目になることを避けられます。ドロワーを開くと title / role / focus の全文と regenerate ボタンが見えます。 |
...
| 横断ドロワー | `/agents`・`/sessions`・セッション詳細の Turns タブ・ターン詳細のスパンツリーで Datadog 風の「行クリック → 右からスライドインする詳細ドロワー」パターンを実現。通常クリックではページ遷移せずに詳細が滑り出し、`Cmd+Click` ではフルページが新タブで開きます。状態は `?drawer=…` クエリに保持されるため、ディープリンクと戻るボタンが期待どおりに動作します。詳細は [`docs/drawer_ja.md`](docs/drawer_ja.md) を参照。 |
| Settings | コレクターのビルド情報と OTel エクスポータの状態。config パスや daemon / hook のインストール導線もこの画面から辿れます。 |
| Session 詳細 | Mission タブ（セッションアークの git graph）を既定表示。セッション単位の rollup、スコープ付き KPI、attention 順に並ぶ全ターンも引き続き利用可能です。旧 Timeline タブは Mission に統合され、phase ノード / operator intervention の branch / TodoWrite のプラン行 / tier-3 forecast tail がひとつの spine（最新 phase が上）に集約されます。実行中ターンがあれば pulse が現在の foothold に載ります。 |
...
| フェーズナラティブ | すべての closed ターンを意味的なフェーズにグルーピングする tier-3 ワーカー。headline、1〜3 文の narrative、key_steps、kind チップ、duration、tool summary を備え、`/session` の Mission git graph にクリック可能なノードとサイドドロワー詳細で表示されます。`Re-chart` ボタン押下時は Sonnet 呼び出し中に spinner と経過秒数（150 秒の safety timeout 付き）が出ます。 |
| Live-status ワーカー | tier-4 ワーカー（Haiku、セッション単位）。実行中セッションについて "currently &lt;verb&gt;-ing &lt;noun&gt;" の 1 行ブラブを更新し続けます。span 挿入ごとに 10 秒の debounce で発火し、各ターミナルの今やっていることが一目で分かります。 |
| Agent サマリーワーカー | エージェント単位の LLM title / role（Haiku）を専用の `agent_summaries` テーブル（`(agent_id, session_id)` キー）に書き込むワーカー。`/agents` カタログは `agent_type` の生文字列ではなく title を先頭に表示するため、並列 main エージェントが同じ見た目になりません。更新は `POST /v1/agents/{id}/summarize`。 |
...
| CLI | `serve`, `init`, `onboard`, `hook`, `daemon`, `status`, `logs`, `open`, `uninstall`, `menubar`, `doctor`, `version`。単一バイナリ、Node / Python ランタイムなし。 |
...
永続化は DuckDB で、`spans` / `logs` / `metric_points` など OTel 互換のテーブルに加え、ダッシュボードの高速読み出し用に非正規化した `sessions` / `turns` / `hitl_events` / `session_rollups` / `agent_summaries` / `interventions` / `task_type_history` を持ちます。`turns` 行は導出カラム `attention_state` / `attention_reason` / `phase` / `recap_json` を、`sessions` 行は session-scoped attention / fleet view 用のカラム（`attention_state` / `current_turn_id` / `current_phase` / `live_state` / `live_status_text`）を保持し、これらは tier-4 live-status ワーカーがセッション実行中に更新し続けます。詳しくは [`docs/architecture_ja.md`](docs/architecture_ja.md) と [`internal/store/duckdb/schema.sql`](internal/store/duckdb/schema.sql) を参照してください。
...
| Tier-3 フェーズナラティブ + forecast | shipped |
| Tier-4 live-status ワーカー（セッションごとの "currently doing X"） | shipped |
| エージェント単位の LLM title + role ワーカー | shipped |
| Mission git graph（Timeline タブから置き換え） | shipped |
| セッション単位 attention + `/v1/sessions/active` fleet エンドポイント | shipped |
...
- **セッション詳細** (`/session/?id=<id>`): 既定で **Mission タブ** が開きます。Mission タブはセッションを縦型の git graph として描画します（最新フェーズが最上段）。LLM が分類したフェーズ (implement / review / debug / plan / test / commit / delegate / explore / other) が spine 上のノードとなり、operator intervention は branch として枝分かれし、TodoWrite のプランは upcoming ノードとして上書きされ、tier-3 forecast は dashed tail として末尾に伸びます。フェーズノードをクリックするとサイドドロワーが開いて全文 narrative・key steps・ツール使用量のバーチャート・フェーズ内のターン一覧が表示されます。
...
macOS なら、ライブステータスアイコンとクイックアクションのためにメニューバーアプリも起動できます。`apogee onboard` は launchd のログイン項目（`dev.biwashi.apogee.menubar`）として登録するので、以降は毎回のログインで自動起動します。インストール導線を直接呼ぶことも可能です:

```sh
apogee menubar install   # macOS のログイン項目として登録（1 度だけ）
apogee menubar &         # もしくは手動でフォアグラウンド起動
```
...
internal/
  ...
  summarizer/       recap (Haiku、ターン単位) / rollup (Sonnet、セッション単位) /
                    narrative (Sonnet、tier-3 フェーズナラティブ) /
                    live-status (Haiku、tier-4 "currently doing X") /
                    agent-summary (Haiku、エージェント単位 title+role) ワーカー
