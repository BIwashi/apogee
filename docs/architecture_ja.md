...
- **agent-summary ワーカー** — `(agent_id, session_id)` ごと、Haiku 層。エージェントの invocation count が前回スナップショットを超えたとき、または 5 分の staleness floor を過ぎたときに reconstructor がジョブを enqueue します。構造化された title / role / focus blob を `agent_summaries` テーブルに書き込みます。手動トリガーは `POST /v1/agents/{id}/summarize`。`/agents` カタログは `agent_type` の生文字列ではなくこの title を先頭に表示します。
