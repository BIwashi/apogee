...
4. 4 層の LLM サマライザ（ターンごとの recap、セッションごとの rollup、tier-3 phase narrative、tier-4 live-status）と、エージェント単位のサマリーワーカーを、ローカルの `claude` CLI 経由で駆動する。
...
collector が公開するルート:
  ...
  /v1/sessions/active                    fleet view: 実行中セッション 1 行ずつ（セッション中心 triage）
  /v1/sessions/attention/counts          attention バケットヒストグラム（turns ではなくセッション単位）
  ...
  /v1/sessions/{id}/narrative            tier-3 phase narrative を手動再生成（POST）
  /v1/sessions/{id}/todos                最新 TodoWrite payload を型付き checklist として返す
  ...
  /v1/agents/recent                      直近のエージェント（main + subagent）、呼び出し回数、LLM の title / role
  /v1/agents/{id}/detail                 横断 drawer 用のエージェントバンドル（agent + parent + children + turns + tool_counts + summary）
  /v1/agents/{id}/summarize              エージェント単位の Haiku title / role ワーカーを POST で再実行
...
| テーブル | 役割 |
|---|---|
| `sessions` | Claude Code セッションごとに 1 行。session-scoped attention + fleet view のカラム（`attention_state` / `attention_reason` / `attention_score` / `current_turn_id` / `current_phase` / `live_state` / `live_status_text` / `live_status_at` / `live_status_model`）は、reconstructor の `bubbleSessionLive` ヘルパーと tier-4 live-status ワーカーが書き戻す。 |
...
| `session_rollups` | セッション単位のナラティブダイジェスト（Sonnet tier-2 rollup。tier-3 narrative ワーカーが同じ JSON blob に `phases[]` + `forecast[]` を追記する） |
| `agent_summaries` | `(agent_id, session_id)` ごとの LLM 生成 title + role + summary blob（agent-summary ワーカー、Haiku）。並列 main エージェントを視覚的に区別するために `/agents` が使う。 |
| `interventions` | operator 由来のメッセージ。queued → claimed → delivered → consumed |
| `task_type_history` | ツール組合せ単位のローリング成功 / 失敗カウント（watchlist に利用） |
| `watchdog_signals` | watchdog ワーカーが検出した異常レコード |
| `model_availability` | ローカル `claude` CLI が受け付ける Claude モデルエイリアスのキャッシュ |
| `user_preferences` | summarizer の言語 / system prompt / model override / UI ノブを入れる汎用 K/V テーブル |
...
### `internal/summarizer` — 4 層の LLM サマライザ + エージェント単位のワーカー

apogee は Anthropic API に直接話しかけません。すべての LLM 呼び出しはローカルの `claude` CLI を経由します。オペレーターの既存の認証・レート制限・コンテキストがそのまま反映されます。

- **Tier 1 — recap ワーカー** — ターンごと、Haiku 層。`Stop` で発火し、`headline` / `key_steps` / `outcome` / `failure_cause` などを持つ構造化 recap JSON を生成し、`turns.recap_json` に書き戻して、ルートにリンクする事後補強 span `claude_code.turn.recap` を発行します。
- **Tier 2 — rollup ワーカー** — セッションごと、Sonnet 層。`SessionEnd` とスケジュール実行で発火し、ナラティブダイジェスト（`headline` / `narrative` / `highlights` / `patterns` / `open_threads`）を `session_rollups.rollup_json` に書き込みます。ダッシュボードのセッション詳細ページはこの行を読みます。チェイニングの契約は [`narrative_ja.md`](narrative_ja.md) を参照。
- **Tier 3 — narrative ワーカー** — セッションごと、Sonnet 層。tier-2 rollup にチェインし、クローズ済みターンを意味的フェーズ（implement / review / debug / plan / test / commit / delegate / explore / other）にグルーピングして、同じ `session_rollups` JSON blob に `phases[]` + `forecast[]` を追記します。手動トリガーは `POST /v1/sessions/:id/narrative`。`/session` の Mission git graph はこの出力を描画します。
- **Tier 4 — live-status ワーカー** — セッションごと、Haiku 層。span 挿入ごとに 10 秒の debounce（`Config.LiveStatusDebounce`）で発火し、"currently &lt;verb&gt;-ing &lt;noun&gt;" の 1 行ブラブを `sessions.live_status_text` に書き込みます。Sessions カタログと Live triage レールが開かずに「今なにをしているか」を伝えられます。`sessions.live_state = 'live'` でガードされるため、idle セッションでは Haiku 呼び出しが走りません。
- **agent-summary ワーカー** — `(agent_id, session_id)` ごと、Haiku 層。enqueue 元は次の 3 経路です: セッション終了（コレクターが `SessionEnd` で `EnqueueAgentSummariesForSession` を呼ぶ）、tier-2 rollup 完了（summarizer サービスが rollup 着地後にそのセッション配下の候補 agent を再スキャン）、手動再実行（`POST /v1/agents/{id}/summarize`）。`process()` 内部では 2 本の skip ガードを適用します — agent の invocation count が `invocation_count_at_generation` を超えている、**または** 既存サマリが 5 分の staleness floor を過ぎていること。構造化された title / role / focus blob を `agent_summaries` テーブルに書き込みます。`/agents` カタログは `agent_type` の生文字列ではなくこの title を先頭に表示します。

全ワーカー共通のフィードバックループ対策として、summarizer は自身の `claude` サブプロセスに `APOGEE_HOOK_SKIP=1` を設定し、apogee hook が自分の telemetry を取り込まないようにしています。`claude` が `PATH` に無い場合は、どのワーカーもログを一度残して黙ってスキップし、ダッシュボードは壊れるのではなく該当パネルが空で描画されます。
