...
agent_summaries  (agent_id, session_id) ごとに 1 行。Haiku 層の title/role
task_type_history ツール組合せ単位のローリング成功 / 失敗カウント
metric_points    OTel メトリックポイント（時系列）
watchdog_signals watchdog ワーカーが検出した異常レコード
model_availability ローカル `claude` CLI が受け付けるモデルエイリアスのキャッシュ
user_preferences  汎用 K/V（summarizer の言語 / system prompt / model override / UI ノブ）
```
...
| `attention_state` | VARCHAR NULL | 代表ターンから bubble-up された session-scoped attention（`intervene_now` / `watch` / `watchlist` / `healthy`）。reconstructor の `bubbleSessionLive` が書く。 |
| `attention_reason` | VARCHAR NULL | 代表ターンからコピーした attention の根拠（人間可読）。 |
| `attention_score` | DOUBLE NULL | セッション中心ソートの tiebreaker。 |
| `current_turn_id` | VARCHAR NULL | Sessions カタログの session カードが deep-link すべきターン。 |
...
**Readers:** Sessions カタログ、セッション詳細、interventions サービス（スコープ判定）、rollup ワーカー、fleet view エンドポイント（`GET /v1/sessions/active`、`GET /v1/sessions/attention/counts`）、コマンドパレット。
...
| `rollup_json` | VARCHAR | ナラティブダイジェスト JSON (構造は下記) |

`rollup_json` blob は tier-2 のナラティブに加え、任意の tier-3
`phases[]` 配列を持ちます:

```jsonc
{
  "headline": "daemon コアをリファクタリング",
  "narrative": "…",
  "highlights": ["…"],
  "patterns": [],
  "open_threads": [],

  // Tier-3 フェーズナラティブ (任意、internal/summarizer/narrative.go が書き込み)
  "phases": [
    {
      "index": 0,
      "started_at": "2026-04-15T09:30:00Z",
      "ended_at":   "2026-04-15T09:38:00Z",
      "headline": "daemon コアを実装",
      "narrative": "launchd / systemd ユニットとテストを含む internal/daemon を追加。",
      "key_steps": ["launchd ユニット追加", "systemd ユニット追加", "テスト green"],
      "kind": "implement",
...
`phases[]` を持たない古いロールアップもそのままパース可能です — Go
`Rollup` 構造体は `omitempty` でマークされ、TypeScript `Rollup`
インターフェースはオプショナルです。完全な tier-3 契約は
[`narrative_ja.md`](narrative_ja.md) を参照してください。

**Writers:** `internal/summarizer/rollup.go` (tier 2),
`internal/summarizer/narrative.go` (tier 3)。
**Readers:** セッション詳細ページの Overview タブ (rollup パネル) と
Mission タブ (git-graph の phase spine)、Sessions カタログのツールチップ、
Live ページ（focus しているセッションの Mission graph インライン表示）。

---

## `agent_summaries`

`(agent_id, session_id)` ごとに 1 行 — `internal/summarizer/agent_summary.go`
（Haiku）が生成するエージェント単位のアイデンティティカードです。
ワーカーは agent の invocation count が前回スナップショットを超えたとき、
または 5 分の staleness floor を過ぎたときにジョブを enqueue します。手動で
更新するには `POST /v1/agents/{id}/summarize`。

| Column | Type | Purpose |
| --- | --- | --- |
| `agent_id` | VARCHAR | エージェント識別子 |
| `session_id` | VARCHAR | 親セッション — 別セッションで同じ agent は別サマリ |
| `title` | VARCHAR | "Investigating CI failures" のような短いラベル（`/agents` の行頭に載る） |
| `role` | VARCHAR | 1 行の責務説明（第 2 行に載る） |
| `summary_json` | VARCHAR | 構造化 blob 全体（`title` / `role` / `focus` / 生成メタデータ） |
| `invocation_count_at_generation` | INTEGER | 増分リフレッシュの発火条件に使う |
| `generated_at` | TIMESTAMP | 書き込み時刻 |
| `model` | VARCHAR | 生成モデルエイリアス |

主キーは `(agent_id, session_id)` で、`UpsertAgentSummary` はリフレッシュ時
に in-place で置換されます。

**Writers:** `internal/summarizer/agent_summary.go`.
**Readers:** `/v1/agents/recent`（TreeView / flat table は `title` を先頭に、
`role` を 2 行目に表示）、`/v1/agents/{id}/detail`（AgentDrawer の summary
セクションに再生成 CTA 付き）。
...
| Key | Default | Meaning |
| --- | --- | --- |
| `summarizer.language` | `"en"` | 全 summarizer tier の出力言語。`"en"` または `"ja"`。 |
| `summarizer.recap_system_prompt` | `""` | tier-1 Haiku recap 命令ブロックに追記される自由テキスト。最大 2048 文字。 |
| `summarizer.rollup_system_prompt` | `""` | tier-2 Sonnet rollup 命令ブロックに追記される自由テキスト。最大 2048 文字。 |
| `summarizer.narrative_system_prompt` | `""` | tier-3 Sonnet phase narrative 命令ブロックに追記される自由テキスト。最大 2048 文字。 |
| `summarizer.recap_model` | `""` | tier-1 recap モデルエイリアスのオーバーライド。空なら `~/.apogee/config.toml` の `[summarizer] recap_model` にフォールバック。 |
| `summarizer.rollup_model` | `""` | tier-2 rollup モデルエイリアスのオーバーライド。空なら同じく config ファイルにフォールバック。 |
| `summarizer.narrative_model` | `""` | tier-3 narrative モデルエイリアスのオーバーライド。空なら config ファイルにフォールバック。 |
