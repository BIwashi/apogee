...
// ReceiveEvent implements POST /v1/events. It accepts a single HookEvent JSON
// body, validates it, applies it through the reconstructor, and returns 202
// on success.
func (h *Handler) ReceiveEvent(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.Reconstructor.Apply(r.Context(), &ev); err != nil {
		...
		// Validation errors from Validate() are plain errors; treat them as 400.
		switch err.Error() {
		case
